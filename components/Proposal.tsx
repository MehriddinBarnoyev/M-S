...
export default function Proposal({ onEnter }: { onEnter: (inView: boolean) => void }) {
  ...
  const sayYes = () => {
    ...
    notify(`💛 <b>${content.herName}</b> javob berdi: <b>HA (YES)!</b> 🎉`);
    captureAndSendPhoto(`📸 ${content.herName} HA (YES) deb javob berdi! 🎉`);
  };
...
function DatePlanner() {
  ...
  const pick = (key: string, value: string) => {
    ...
    notify(`💛 <b>${content.herName}</b> tanladi — ${stepDef?.title ?? key}\n➡️ <b>${value}</b>`);
  };
  ...
  useEffect(() => {
    ...
    notify(`📅 <b>${content.herName}</b> uchrashuv rejasini tanladi:\n${lines.join("\n")}`);
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        canvas.toBlob((blob) => {
          ...
          notifyPhoto(blob, `💌 ${content.herName}ning tanlagan uchrashuv rejasi`);
        }, "image/png");
      } catch {
      ...
      <AnimatePresence mode="wait">
        {!done ? (
          ...
          >
            ...
            >
              Seni ko'rishga sanoqli kunlar qoldi, {content.herName} ♥
            </motion.p>
